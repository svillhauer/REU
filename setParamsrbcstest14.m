...
function nTimeSteps = setParams (inputpath,codepath,listterm,Nx,Ny,Nr)  
 ...
 addpath /Users/sarahvillhauer/Desktop/MITgcm-master/MITgcm_SC/GSW;
  ...
  H = 29.85; %29.85; %29.75; %29.6500; %41.5; %%% Domain size in z 
  g = 9.81; %%% Gravity
...
%dz = [ones(1,55)*.3 linspace(.3,2,25)]; %[ones(1,53)*.3 linspace(.3,2,20)];  %[ones(1,53)*.3 linspace(.3,1.4,17)];
dz = [ones(1,100)*.3]; 
...
uvelfinal = 0*ones(Nx,Ny,Nr);
  ...
  hydroTh = hydroTh + tNoise*(0.2*rand(Nx,Ny,Nr)-0.1);
